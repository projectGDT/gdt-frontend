...
export const ModList = (props: {versionId: string, pageSize?: number}) => {
  // mod 图标的背景颜色
  ...
  const fetchModpackDependencies = () => {
    ...
    fetch(`https://api.modrinth.com/v2/version/${version}`, { method: "GET", headers: { "User-Agent": "github.com/projectGDT (contact@gdt.pub)" }})
      .then(response => {
        if (response.ok) 
  ...
  const loadPageOfMods = (idx: number) => {
    ...
    Promise.all(dependencies.current
      ...
      .map((item) => {return fetchModInfo(item.project_id, item.version_id, item.file_name)}))
  ...
  const fetchModInfo = async (project_id: string, version_id: string, file_name: string) => {
    ...
    if (!project_id || !version_id) {
      const modinfo: ModInfo = {
        file_name: file_name,
        icon_url: "https://pic.imgdb.cn/item/65cc66fd9f345e8d03f985f4.png" // 默认图标地址
      };
      return modinfo;
    }

    // 否则去 fetch
    const fetchProject = fetch(`https://api.modrinth.com/v2/project/${project_id}`, { method: "GET", headers: { "User-Agent": "github.com/projectGDT (contact@gdt.pub)" }});
    const fetchVersion = fetch(`https://api.modrinth.com/v2/version/${version_id}`, { method: "GET", headers: { "User-Agent": "github.com/projectGDT (contact@gdt.pub)" }});
  ...
  function Skeletons() {
    ...
    return <List sx={{display: "flex", flexDirection: "column", gap: 0.4}}>{ // gap 是为了消除 skeleton 之间距离比 listbuttons 窄导致的伸缩，不知道是否有更好的方法
      Array.from({length: countToLoad}).map((item, idx) => {
        return (
            <ListItem key={idx}>
              <ListItemAvatar>
  ...
  return (<Box sx={{display: "flex", flexDirection: "column", gap: 2}}>
        ...
        (item) => {
          return (
              <ListItemButton key={modInfo.indexOf(item)}>
                <ListItemAvatar>
                  <Avatar src={item.icon_url} style={{width: 48, height: 48}} sx={{bgcolor: iconBgColor}} variant="rounded"/>
                </ListItemAvatar>
                <ListItemText 
    ...
    <Box sx={{display: "flex", flexDirection: "row", justifyContent: "space-between"}}>
      { (!loadingMods && nowModPage > 0) ? 
        <IconButton onClick={() => {if (nowModPage > 0) setNowModPage((prev) => prev - 1)}}>
          <KeyboardArrowLeft/>
        </IconButton> :
        <IconButton disabled>
          <KeyboardArrowLeft/>
      ...
      { (!loadingMods && (nowModPage + 1) * modPageSize < dependencies.current.length) ? 
        <IconButton onClick={() => {if ((nowModPage + 1) * modPageSize < dependencies.current.length) setNowModPage((prev) => prev + 1)}}>
          <KeyboardArrowRight/>
        </IconButton> :
        <IconButton disabled>
          <KeyboardArrowRight/>
